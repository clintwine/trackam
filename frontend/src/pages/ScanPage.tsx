      ...
      <main className="flex-1 flex flex-col px-4 py-6 max-w-md mx-auto w-full">
            ...
            <div className="rounded-lg border border-purple-200 bg-purple-50 p-4">
              ...
              <div className="space-y-1.5 max-h-40 overflow-y-auto">
                {batchInfo.shipments.map((s) => (
                  <div key={s.shipmentId} className="rounded-md bg-white/70 border border-purple-100 px-2.5 py-1.5">
                    {s.waybillNumber && (
                      <p className="text-[11px] font-mono font-semibold text-purple-900">{s.waybillNumber}</p>
                    )}
                    <p className="text-[11px] text-purple-700 truncate">{s.goodsDescription}</p>
                    {s.pickupLocation && s.deliveryLocation && (
                      <p className="text-[10px] text-purple-500 flex items-center gap-1 mt-0.5">
        ...
        {phase === "success" && confirmation && (
          <div className="flex-1 flex flex-col items-center justify-center text-center gap-5">
            ...
            {"confirmedCount" in confirmation ? (
              <div className="w-full rounded-lg border border-border bg-white p-4 text-left space-y-1.5">
                ...
                {confirmation.proofHashes.slice(0, 3).map((ph) => (
                  <p key={ph.shipmentId} className="font-mono text-[10px] break-all text-foreground">
                    {ph.proofHash.slice(0, 20)}…
                  </p>
                ))}
                {confirmation.proofHashes.length > 3 && (
                  <p className="text-[10px] text-muted-foreground">+{confirmation.proofHashes.length - 3} more</p>
                )}
                <p className="text-[11px] text-muted-foreground pt-1">Fee deducted: ₦{confirmation.totalFee.toFixed(2)}</p>
              </div>
            ) : (
              <div className="w-full rounded-lg border border-border bg-white p-4 text-left space-y-2">
                ...
                <p className="font-mono text-xs break-all text-foreground">{(confirmation as HandoverConfirmation).proofHash}</p>
  ...
  return (
    <>
      ...
      <div>
        ...
        {gpsStatus === "fetching" && <p className="text-xs text-muted-foreground flex items-center gap-1.5"><Loader2 className="h-3 w-3 animate-spin" /> Fetching…</p>}
        ...
        {gpsStatus === "denied" && <p className="text-xs text-amber-700">Location access denied — continuing without GPS.</p>}
      </div>
    </>
  );
}
